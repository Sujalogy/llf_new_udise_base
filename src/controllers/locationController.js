...
exports.getStates = async (req, res) => {
  try {
    const data = await locationModel.getAllStates();
    res.json(data);
  } catch (e) {
...
exports.getDistricts = async (req, res) => {
  try {
    const data = await locationModel.getDistrictsByState(req.params.stcode11);
    res.json(data);
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
...
exports.getSyncedDistricts = async (req, res) => {
  try {
    ...
    const data = await locationModel.getSyncedDistricts(
      req.params.stcode11,
      yearDesc
    );
    res.json(data);
